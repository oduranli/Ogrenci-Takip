...
namespace OzgurYazilim.OgrenciTakip.UI.Win.UserControls.UserControl.TahakkukEditFormTable
{
    public partial class OdemeBilgileriTable : BaseTablo
    {
        ...
        {
            tablo.GridControl.DataSource = ((OdemeBilgileriBll)Bll)
                .List(x => x.TahakkukId == OwnerForm.Id)
                .ToBindingList<OdemeBilgileriL>();
        }
        protected override void HareketEkle()
        {
            ...
            if (!ShowEditForms<TopluOdemePlaniEditForm>.ShowDialogEditForm(source, OwnerForm.Id, BakiyeHesapla(),
                ((TahakkukEditForm)OwnerForm).txtKayitTarihi.DateTime.Date, dahaOnceGirilenTaksitSayisi)) return;
        ...
        protected internal override bool HataliGiris()
        {
            ...
            for (int i = 0; i < tablo.DataRowCount; i++)
            {
                ...
                if (entity.Tutar == 0)
                {
                    ...
                    tablo.SetColumnError(colTutar, "Belge tutarı 0 (Sıfır) olamaz!");
                }
                switch (entity.OdemeTipi)
                {
                    case OdemeTipi.Cek:
                        ...
                        if (string.IsNullOrEmpty(entity.BelgeNo))
                        {
                            ...
                            tablo.SetColumnError(colBelgeNo, "Belge numarası girmelisiniz!");
                        }
                        if (string.IsNullOrEmpty(entity.AsilBorclu))
                        {
                            tablo.FocusedRowHandle = i;
                            tablo.FocusedColumn = colAsilBorclu;
                            tablo.SetColumnError(colAsilBorclu, "Asıl borçlu adını girmelisiniz!");
                        }
                        break;
        ...
        protected override void Tablo_FocusedColumnChanged(object sender, FocusedColumnChangedEventArgs e)
        {
            ...
            else if (e.FocusedColumn == colVade)
            {
                ...
                repositoryVade.MaxValue = AnaForm.DonemParemetreleri.MaksimumTaksitTarihi;
            }
        }
    }
}
